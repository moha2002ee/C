...
void ApplicHoraireWindow::on_actionImporterProfesseurs_triggered()
{
    ...
    string fichierName = dialogInputFileForLoad("Quelle est le fichier que tu veux ajouter ? ");
    if (!fichierName.empty())
    {

        int nb = Timetable::getInstance().importProfessorsFromCsv(fichierName);
        cout << nb << " prof importés avec succès depuis " << fichierName << std::endl;
    }
    updateAllTables();
}
...
void ApplicHoraireWindow::on_actionImporterGroupes_triggered()
{
    ...
    if (!fichierName.empty())
    {
        ...
        int nb = Timetable::getInstance().importGroupsFromCsv(fichierName);
        cout << nb << " groupe importés avec succès depuis " << fichierName << std::endl;
    }
    updateAllTables();
}
...
void ApplicHoraireWindow::on_actionImporterLocaux_triggered()
{
    ...
    if (!fichierName.empty())
    {
        ...
        int nb = Timetable::getInstance().importClassroomsFromCsv(fichierName);
        cout << nb << " groupe importés avec succès depuis " << fichierName << std::endl;
    }
    updateAllTables();
}
...
void ApplicHoraireWindow::on_actionExporterProfesseur_triggered()
{
    ...
    // TO DO (Etape 12)
    int resultat = dialogInputInt("Exporter horaire --> Item Professeur", "C'est quelle prof ");
    if (resultat <= 0)
    {
        return;
    }
    ...
    {
        Timetable::getInstance().exportProfessorTimetable(resultat);
    }
}
...
void ApplicHoraireWindow::on_actionExporterGroupe_triggered()
{
    ...
    // TO DO (Etape 12)
        int resultat = dialogInputInt("Exporter horaire --> Item Groupe", "C'est quelle Groupe ");
    if (resultat <= 0)
    {
        return;
    }
    ...
    {
        Timetable::getInstance().exportGroupTimetable(resultat);
    }
}
...
void ApplicHoraireWindow::on_actionExporterLocal_triggered()
{
    ...
    // TO DO (Etape 12)
        int resultat = dialogInputInt("Exporter horaire --> Item Local", "C'est quelle Local ");
    if (resultat <= 0)
    {
        return;
    }
    ...
    {
        Timetable::getInstance().exportLocalTimetable(resultat);
    }
}
