...
WindowClient::WindowClient(QWidget *parent) : QMainWindow(parent), ui(new Ui::WindowClient)
{
  ...
  {
    perror("il y a une probleme pour la creation de la file de message ");
    exit(1);
  }
  ...
  {
    perror("il y a une erreur pour le msgsnd");
    exit(1);
  }
...
void WindowClient::closeEvent(QCloseEvent *event)
{
  ...
  if (logged == true)
  {
    ...
    if (msgsnd(idQ, &m, sizeof(MESSAGE) - sizeof(long), 0) == -1)
    {
      perror("il y a une erreur avec le msgsnd");
      exit(1);
    }
  }
  m.type = 1;
  ...
  if (msgsnd(idQ, &m, sizeof(MESSAGE) - sizeof(long), 0) == -1)
  {
    perror("il y a une porbleme avec le msgsnd");
    exit(1);
  }
